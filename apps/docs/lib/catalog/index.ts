...
export type CatalogPage = {
  url: string;
  data: { title?: string | undefined; description?: string | undefined; catalog?: CatalogItem | undefined };
};

export type Catalog = {
  categories: Array<{ id: string; name: string; description?: string | undefined }>;
  items: Array<CatalogItem & { title: string; description: string; url: string }>;
};
...
export function buildCatalog(pages: readonly CatalogPage[]): Catalog {
  ...
  const items = pages.flatMap((page) => {
    ...
    if (!page.data.title || !page.data.description) {
      throw new Error(`Catalog item ${item.id} requires its MDX title and description`);
    }
    ...
    return [{ ...item, title: page.data.title, description: page.data.description, url: item.docsUrl ?? page.url }];
  });
