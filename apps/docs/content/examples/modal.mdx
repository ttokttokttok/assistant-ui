...
catalog: { id: modal, kind: example, category: { id: ui-patterns, name: UI Patterns }, tags: [Modal, Example], prompt: Open the floating modal chat example., gradient: from-rose-500/40 via-orange-500/30 to-amber-400/20, image: /screenshot/examples/modal.png, sourcePath: apps/docs/components/docs/samples/assistant-modal.tsx, preview: { status: live, url: /xulux-preview/modal }, tech: { framework: React, runtime: assistant-ui, frontendPattern: Assistant modal }, env: [], capabilities: { openInChat: true, agentProfile: modal } }
---
