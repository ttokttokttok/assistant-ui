---
title: Gemini Style Assistant
description: A Gemini-inspired assistant surface with suggested starter prompts, rounded composer controls, message actions, and lightweight visual styling.
catalog: { id: gemini, kind: example, order: 4, category: { id: assistant-ui-demos, name: Assistant UI Demos }, tags: [assistant-ui, Gemini, clone, chat], prompt: Open the Gemini Style Assistant demo., gradient: from-blue-500/35 via-sky-300/25 to-rose-300/20, image: /screenshot/examples/gemini.png, examplesCard: { title: Gemini Clone, description: Customized colors and styles for a Gemini look and feel., image: /screenshot/examples/gemini.png }, sourcePath: apps/docs/components/examples/gemini.tsx, preview: { status: live, url: /demos/gemini }, tech: { framework: Next.js, runtime: assistant-ui + AI SDK, frontendPattern: Fixed demo }, env: [{ name: OPENAI_API_KEY, required: false, secret: true, description: Optional. Enables live AI responses in the downloaded starter app. }], capabilities: { openInChat: true, downloadProfile: demo-gemini, agentProfile: demo-gemini } }
---
